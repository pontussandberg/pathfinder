...
function checkForOptions() {
    if (currentPos) {
        for (let i = 0; i < board.length; i++) {
            ...
            if (board[i].col === currentPos.col || board[i].col - 1 === currentPos.col || board[i].col + 1 === currentPos.col) {
                isCol = true;
            }
            ...
            if (board[i].row + 1 === currentPos.row && board[i].col + 1 === currentPos.col) {
                isRow = false;
                isCol = false;
            }
            if (board[i].row - 1 === currentPos.row && board[i].col - 1 === currentPos.col) {
...
function stats(e) {
    displayStats.innerHTML = `X: ${e.offsetX}   Y: ${e.offsetY}`;
}
